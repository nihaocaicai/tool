...
public class ArrangementController {
     ...
     *根据id查询用户的全部日记记录
     * */
    @RequestMapping(value = "/user/arrangements/all", method = {RequestMethod.GET}, produces="application/json;charset=UTF-8")
    @ResponseBody
    public String findUserById(String id) throws ClassNotFoundException, NoSuchMethodException, IllegalAccessException, InvocationTargetException {
        int uid = Integer.parseInt(id);
        List<Arrangement> arrangement = arrangementService.findArrangeByUserId(uid);
//		加入转换的数据，类中所在的日期方法名,类的对象
        HashMap<String,List<Arrangement>> map = ResponseData.<Arrangement>responseData(arrangement,"getArrange_date",new Arrangement());
    ...
    @ResponseBody
    public String addArrangement(@ModelAttribute("id") String id,@ModelAttribute("user_arrangement_add") String user_arrangement_add) {
...
//        这里获取formid
        ...
        Date arrange_if_prompt_date = Date.valueOf(JSON.parseObject(user_arrangement_add).getString("arrange_if_prompt_date"));
        Time arrange_if_prompt_time = Time.valueOf(JSON.parseObject(user_arrangement_add).getString("arrange_if_prompt_time"));
//    	根据信息去添加相关安排
        arrangementService.insertArrange(new Arrangement(uid,arrange_content,arrange_place,arrange_date,arrange_time,
        arrange_if_prompt,arrange_if_prompt_date,arrange_if_prompt_time));
    ...
    @ResponseBody
    public String modifyArrangement(@ModelAttribute("id") String id,@ModelAttribute("user_arrangement_modify") String user_arrangement_modify) {
        int uid = Integer.parseInt(id);
...
//        这里获取更新再次提交的formid
        ...
        Date arrange_if_prompt_date = Date.valueOf(JSON.parseObject(user_arrangement_modify).getString("arrange_if_prompt_date"));
        Time arrange_if_prompt_time = Time.valueOf(JSON.parseObject(user_arrangement_modify).getString("arrange_if_prompt_time"));
//    	根据信息去更新相关安排
        arrangementService.updateArrange(new Arrangement(arrange_id,uid,arrange_content,arrange_place,arrange_date,arrange_time,
                arrange_if_prompt,arrange_if_prompt_date,arrange_if_prompt_time));
     ...
     *根据id删除考研日记
     */
    @RequestMapping(value = "/user/arrangements/delete", method = {RequestMethod.GET}, produces="application/json;charset=UTF-8")
    @ResponseBody
